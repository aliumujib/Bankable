...
class FetchGTBankOtherBanksFirstPageContract :
    ...
    override fun parseResult(resultCode: Int, intent: Intent?): USSDResult<List<BankMenuModel>> {
        if (resultCode != Activity.RESULT_OK) return USSDResult(context.getString(R.string.error_fetching_bank_acc_balance))
        if (intent == null) return USSDResult(context.getString(R.string.error_fetching_bank_acc_balance))
